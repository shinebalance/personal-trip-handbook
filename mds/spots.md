---
type: collection
entries:
  - id: animate-hongdae
    location:
      lat: 37.5580543
      lng: 126.9260821
      kind: place
      sourceUrl: 'https://www.waze.com/live-map/directions/kr/seoul/animate-hongdae?to=place.ChIJIS5I20WZfDUR94oKsom82WA'
      checkedAt: '2026-09-22'
    type: spot
    title:
      ja: Animate 弘大
      ko: 애니메이트 홍대
      en: Animate Hongdae
    koreanName: 애니메이트 홍대
    description:
      ja: 現地のアニメショップは見たい。
      ko: 현지 애니메이션 숍을 둘러보고 싶다.
      en: Explore an anime shop in Seoul.
    area: hongdae
    category: anime
    address: 서울특별시 마포구 양화로 188 AK플라자 5층
    website: 'https://www.animate.co.jp/shop/hongdae/'
    sourceUrl: 'https://www.animate.co.jp/shop/hongdae/'
    verifiedAt: '2026-09-22'
  - id: thanks-books
    location:
      lat: 37.5488222
      lng: 126.9176773
      kind: place
      sourceUrl: 'https://www.koreaetour.com/thanks-books/'
      checkedAt: '2026-09-22'
    type: spot
    title:
      ja: THANKS BOOKS
      ko: 땡스북스
      en: THANKS BOOKS
    koreanName: 땡스북스
    description:
      ja: 独立系書店で、新しい本との出会いを。
      ko: 독립서점에서 새로운 책을 만나기.
      en: Browse an independent bookstore.
    area: hongdae
    category: books
  - id: object
    location:
      lat: 37.5557150814061
      lng: 126.929810541616
      kind: place
      sourceUrl: 'https://ontrip.kr/en/travel-guides/details/4030833'
      checkedAt: '2026-09-22'
    type: spot
    title:
      ja: object
      ko: 오브젝트
      en: object
    koreanName: 오브젝트
    description:
      ja: 今どきの文具を探したい。
      ko: 요즘 감성의 문구를 찾아보기.
      en: Look for contemporary stationery.
    area: hongdae
    category: design
    priority: if-time
  - id: gimbab-records
    location:
      lat: 37.55826
      lng: 126.92476
      kind: place
      sourceUrl: 'https://mapcarta.com/N4968397709'
      checkedAt: '2026-09-22'
    type: spot
    title:
      ja: Gimbab Records
      ko: 김밥레코즈
      en: Gimbab Records
    koreanName: 김밥레코즈
    description:
      ja: レコードをゆっくり掘りたい。
      ko: 레코드를 천천히 둘러보기.
      en: Take time to dig through the records.
    area: hongdae
    category: music
    priority: if-time
  - id: beluga-music
    type: spot
    title:
      ja: Beluga Music
      ko: 벨루가 뮤직
      en: Beluga Music
    koreanName: 벨루가 뮤직
    description:
      ja: もう一軒、気になるレコード店。
      ko: 관심 있는 또 하나의 레코드 숍.
      en: Another record shop on the list.
    area: hongdae
    category: music
    priority: if-time
  - id: figure-friends
    location:
      lat: 37.5566069
      lng: 126.9250454
      kind: place
      sourceUrl: 'https://www.waze.com/live-map/directions/kr/seoul/figure-friends?to=place.ChIJ4-MqsMSYfDURdbMlub1IS0w'
      checkedAt: '2026-09-22'
    type: spot
    title:
      ja: Figure Friends
      ko: 피규어 프렌즈
      en: Figure Friends
    koreanName: 피규어 프렌즈
    description:
      ja: 気になるフィギュアを探す。
      ko: 마음에 드는 피규어 찾아보기.
      en: Browse for figures.
    area: hongdae
    category: anime
    priority: if-time
  - id: your-mind
    type: spot
    title:
      ja: YOUR-MIND
      ko: 유어마인드
      en: YOUR-MIND
    koreanName: 유어마인드
    description:
      ja: ZINEや独立出版を見たい。
      ko: 진과 독립출판물을 구경하고 싶다.
      en: Discover zines and independent publications.
    area: yeonhui
    category: books
    address: 서울시 서대문구 연희로 122 경교빌딩 302
    website: 'https://www.your-mind.com/yourmind/offline.html'
    sourceUrl: 'https://www.your-mind.com/yourmind/offline.html'
    verifiedAt: '2026-09-22'
  - id: international-electronics
    type: spot
    title:
      ja: 国際電子センター
      ko: 국제전자센터
      en: International Electronics Center
    koreanName: 국제전자센터
    description:
      ja: 現地の電気街は見たい。所在地を確認してから予定に入れる。
      ko: 현지 전자상가를 보고 싶다. 위치를 확인한 뒤 일정에 넣기.
      en: Explore a local electronics center. Confirm the location before planning a visit.
    area: unconfirmed
    category: electronics
  - id: yongsan-electronics
    type: spot
    title:
      ja: 龍山の電気街
      ko: 용산 전자상가
      en: Yongsan electronics district
    koreanName: 용산 전자상가
    description:
      ja: 電気街を歩いてみたい。
      ko: 전자상가를 걸어보고 싶다.
      en: Take a walk around the electronics district.
    area: yongsan
    category: electronics
  - id: storage-book-film
    type: spot
    title:
      ja: Storage Book & Film
      ko: 스토리지북앤필름
      en: Storage Book & Film
    koreanName: 스토리지북앤필름
    description:
      ja: 龍山に行くなら、ぜひ寄りたい本屋。
      ko: 용산에 가면 꼭 들러보고 싶은 서점.
      en: A bookstore to visit if I make it to Yongsan.
    area: yongsan
    category: books
    priority: if-time
  - id: kyobo-gwanghwamun
    location:
      lat: 37.570975
      lng: 126.9777592
      kind: place
      sourceUrl: 'https://kr.near-place.com/kyobo-bookstore-gwanghwamun-1-jong-ro-jongno-1il2i3sam4sa-jongno-gu'
      checkedAt: '2026-09-22'
    type: spot
    title:
      ja: 教保文庫 光化門店
      ko: 교보문고 광화문점
      en: Kyobo Book Centre
    koreanName: 교보문고 광화문점
    description:
      ja: 大型書店で技術書もチェック。
      ko: 대형 서점에서 기술 서적도 살펴보기.
      en: 'Explore a large bookstore, including its technical books.'
    area: gwanghwamun
    category: books
    address: '서울특별시 종로구 종로 1, 교보생명빌딩 지하 1층'
    website: 'https://store.kyobobook.co.kr/store-info/001'
    sourceUrl: 'https://store.kyobobook.co.kr/store-info/001'
    verifiedAt: '2026-09-22'
  - id: hottracks
    type: spot
    title:
      ja: HOTTRACKS
      ko: 핫트랙스 광화문점
      en: HOTTRACKS
    koreanName: 핫트랙스 광화문점
    description:
      ja: 文具とCD。買い物リストもここで確認。
      ko: 문구와 CD. 여기서 쇼핑 목록도 확인하기.
      en: Stationery and CDs. Check the shopping list here too.
    area: gwanghwamun
    category: design
    related:
      - stationery
      - cds
  - id: lotte-mart
    type: spot
    title:
      ja: ロッテマート
      ko: 롯데마트
      en: Lotte Mart
    koreanName: 롯데마트
    description:
      ja: 現地のスーパーマーケットをのぞく。
      ko: 현지 대형마트 둘러보기.
      en: Browse a local supermarket.
    area: anywhere
    category: shopping
    priority: if-found
  - id: olive-young
    type: spot
    title:
      ja: オリーブヤング
      ko: 올리브영
      en: Olive Young
    koreanName: 올리브영
    description:
      ja: お土産を探したい。
      ko: 선물을 찾아보고 싶다.
      en: Look for gifts to bring home.
    area: anywhere
    category: shopping
    priority: if-found
  - id: convenience-stores
    type: spot
    title:
      ja: CU / GS25
      ko: CU / GS25
      en: CU / GS25
    koreanName: CU / GS25
    description:
      ja: 韓国のコンビニ文化を体験。
      ko: 한국 편의점 문화 경험하기.
      en: Experience Korean convenience-store culture.
    area: anywhere
    category: shopping
    priority: if-found
  - id: lotte-world-tower
    location:
      lat: 37.51255
      lng: 127.10268
      kind: place
      sourceUrl: 'https://mapcarta.com/N12520558001'
      checkedAt: '2026-09-22'
    type: spot
    title:
      ja: ロッテワールドタワー
      ko: 롯데월드타워
      en: Lotte World Tower
    koreanName: 롯데월드타워
    description:
      ja: 時間に余裕があれば行ってみたい。
      ko: 시간이 남으면 가보고 싶다.
      en: Visit if there is enough time.
    area: jamsil
    category: walk
    priority: if-time
  - id: naksan-park
    location:
      lat: 37.58055
      lng: 127.00721
      kind: area
      sourceUrl: 'https://mapcarta.com/W229027411'
      checkedAt: '2026-09-22'
    type: spot
    title:
      ja: 駱山公園
      ko: 낙산공원
      en: Naksan Park
    koreanName: 낙산공원
    description:
      ja: KPop Demon Huntersのメモから、気になる場所。
      ko: KPop Demon Hunters 메모에서 관심이 생긴 장소.
      en: A place on my list because of KPop Demon Hunters.
    area: jongno
    category: walk
    priority: if-time
  - id: myeongdong
    location:
      lat: 37.5642
      lng: 126.98377
      kind: area
      sourceUrl: 'https://mapcarta.com/25947090'
      checkedAt: '2026-09-22'
    type: spot
    title:
      ja: 明洞
      ko: 명동
      en: Myeongdong
    koreanName: 명동
    description:
      ja: 時間があったら街歩き。
      ko: 시간이 있으면 동네 산책하기.
      en: Walk around the neighborhood if time allows.
    area: myeongdong
    category: walk
    priority: if-time
  - id: ept
    type: spot
    title:
      ja: ept 聖水
      ko: 이피티 성수
      en: ept Seongsu
    koreanName: 이피티 성수
    description:
      ja: スニーカーを見に行きたい。
      ko: 운동화를 보러 가고 싶다.
      en: Look for sneakers in Seongsu.
    area: seongsu
    category: shopping
  - id: mega-coffee
    type: spot
    title:
      ja: MEGA Coffee
      ko: 메가MGC커피
      en: MEGA Coffee
    koreanName: 메가MGC커피
    description:
      ja: 散歩の途中にコーヒー休憩。
      ko: 산책 중 커피 한 잔.
      en: A coffee break along the way.
    area: anywhere
    category: cafe
  - id: fritz-coffee
    type: spot
    title:
      ja: Fritz Coffee Company
      ko: 프릳츠
      en: Fritz Coffee Company
    koreanName: 프릳츠
    description:
      ja: 気になっているコーヒー店。支店はこれから選ぶ。
      ko: 관심 있는 커피숍. 방문할 지점은 나중에 정하기.
      en: A coffee shop I want to try. Branch to be decided.
    area: anywhere
    category: cafe
    sourceUrl: 'https://typica.coffee/ja/narratives/roasters/fritz-coffee-company/'
  - id: changee
    type: spot
    title:
      ja: CHANGEE
      ko: 차지 CHANGEE
      en: CHANGEE
    koreanName: 차지 CHANGEE
    description:
      ja: 中国発のお茶ブランド。韓国の店舗を調べてみたい。
      ko: 중국에서 시작한 차 브랜드. 한국 매장을 찾아보고 싶다.
      en: A tea brand from China. Look into its Korean locations.
    area: anywhere
    category: cafe
---
