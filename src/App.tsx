...
export default function App() {
  ...
  const [ratings, setRatings] = useState<Record<string, number>>(() => { const value = stored<unknown>('seoul-ratings', {}); return value && typeof value === 'object' && !Array.isArray(value) ? Object.fromEntries(Object.entries(value).filter(([, rating]) => typeof rating === 'number' && Number.isInteger(rating) && rating >= 0 && rating <= 5)) : {}; });
  ...
  useEffect(() => { save('seoul-ratings', ratings); }, [ratings]);
  ...
  function reset() { setSearch(''); setArea('all'); setCategory('all'); setPriority('all'); setMinRating(0); setOnlyChecked(false); }
  ...
  function setRating(id: string, rating: number) { setRatings(value => ({ ...value, [id]: rating })); }
  function ratingFor(entry: Entry) { return ratings[entry.id] ?? entry.initialRating ?? 0; }
  ...
  const filtered = entries.filter(entry =>
    ...
    (priority === 'all' || entry.priority === priority) && ratingFor(entry) >= minRating && (!onlyChecked || checked.includes(entry.id)) &&
    ...
    <main id="main" className="shell" tabIndex={-1}>
      ...
      {isList && <section id="places" className="places-section">
        ...
        <div className="filter-panel">
          ...
          <div className="filter-bottom"><div className="category-list" role="group" aria-label={t.allCategories}>{['all', ...categories].map(cat => <button aria-pressed={category === cat} className={category === cat ? 'active' : ''} key={cat} onClick={() => setCategory(cat)}>{cat === 'all' ? t.allCategories : t.categories[cat] || cat}</button>)}</div><div className="rating-filter" role="group" aria-label={t.myPriority}>{([0, 4, 5] as const).map(value => <button key={value} aria-pressed={minRating === value} onClick={() => setMinRating(value)}>{value === 0 ? t.ratingFilterAll : value === 4 ? t.ratingFilterFour : t.ratingFilterFive}</button>)}</div><label className="checked-filter"><input type="checkbox" checked={onlyChecked} onChange={event => setOnlyChecked(event.target.checked)} /><span>{t.showCompleted}</span></label></div>
        </div>
